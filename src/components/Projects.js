...
function Projects() {
  ...
  return (
    <Grid container spacing={2} sx={{ padding: 2 }}> {/* Grid container for projects */}
      {projects.map((project, index) => ( // Mapping through projects
        <Grid item xs={12} sm={6} md={4} key={index}> {/* Grid item for each project */}
          <Card sx={{ '&:hover': { boxShadow: 10 } }}> {/* Card */}
            <CardContent> {/* Card content */}
              <Typography variant="h5" component="h2" color="primary">{project.name}</Typography> {/* Project name */}
              <Typography color="textSecondary">{project.description}</Typography> {/* Project description */}
            </CardContent>
            <CardActions> {/* Card actions */}
              <Button size="small" color="secondary" target="_blank" rel="noopener noreferrer" href={project.link}>Link to Project</Button> {/* Link to project button */}
            </CardActions>
          </Card>
        </Grid>
      ))}
